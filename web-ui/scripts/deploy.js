...
function validateEnvironment(env) {
  ...
  if (!config.supabaseUrl || !config.supabaseKey) {
    log(
      `❌ Variáveis de ambiente não configuradas para ${config.name}`,
      'error'
    )
    log(
      'Configure STAGING_SUPABASE_URL/STAGING_SUPABASE_ANON_KEY ou PRODUCTION_SUPABASE_URL/PRODUCTION_SUPABASE_ANON_KEY',
      'warning'
    )
    process.exit(1)
  }
...
function checkGitStatus() {
  try {
    ...
    if (status.trim()) {
      ...
      if (!isDev) {
        log(
          '❌ Commit suas mudanças antes do deploy ou use --allow-dirty',
          'error'
        )
        process.exit(1)
      }
